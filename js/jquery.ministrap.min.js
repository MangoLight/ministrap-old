...
a("body").miniMobileMenu();a(".date-selector").each(function(){a(this).miniDatePicker({type:a(this).attr("data-type"),language:a(this).attr("data-language")})})})})})(jQuery);
...
c.indicators&&"slide"==c.mode&&(d.attr("data-current-slide",0),g(b,0))});b.bind("stop",function(){clearInterval(n)});b.bind("slideTo",function(a,e){0<b.find("li").eq(e).length&&0<=e&&(d.attr("data-current-slide",e),c.indicators&&g(b,e),"slide"==c.mode&&d.stop().animate({"margin-left":-1*e*b.width()}),"fade"==c.mode&&m.stop().animate({opacity:0}).eq(e).stop().animate({opacity:1}))});b.bind("slidePrev",function(){var a=d.attr("data-current-slide"),c=b.find("li").length;0==a?a=c-1:a--;b.trigger("slideTo",
...
(function(a){a.fn.miniNav=function(k){function g(b){var c=null;b.find("li a").each(function(){try{a(a(this).attr("href")).offset().top<=a(document).scrollTop()+100&&(c=a(this))}catch(b){}});c||(c=b.find("li a").eq(0));b.find("li.active").removeClass("active");c.parent().addClass("active")}function f(b){b.css({position:"static",top:"auto",width:"auto",height:"auto"}).removeClass("fixed-done");if(665<=a(window).width())if(b.attr("data-top",b.offset().top-parseInt(b.css("margin-top"))).css({position:"fixed",
...
e):e="0000";f=e+"-"+g+"-"+f}"datetime"==c.type&&(f+=" ");"date"!=c.type&&(g=parseInt(d.find(".hours .active").text()),e=parseInt(d.find(".minutes .active").text()),12==c.time_format&&(12==g&&(g=0),d.find(".part .active").is(":last-child")&&(g+=12)),10>g&&(g="0"+g),10>e&&(e="0"+e),f+=g+":"+e+":00");a.val(f)}}var c=a.extend({type:"date",language:navigator.language},k);return this.each(function(){"date"!=c.type&&"datetime"!=c.type&&"time"!=c.type&&(c.type="date");c.language||(c.language="en");for(var b=
new Date("1987-10-22 19:20:00"),d=b.toLocaleDateString(c.language),k=[".","/","-"],n,e=0;e<k.length;e++)if(-1!=d.indexOf(k[e])){n=d.split(k[e]);break}"object"!=typeof n&&(n=[10,22,1987]);c.date_format="";for(e=0;3>e;e++)1987==n[e]?c.date_format+="Y":10==n[e]?c.date_format+="M":22==n[e]&&(c.date_format+="D");d=b.toLocaleTimeString(c.language);-1!=d.indexOf("PM")?c.time_format=12:c.time_format=24;var l=a(this),h=a("<div>").addClass("minidateslector_popup").appendTo("body"),q;g(l,h);l.on("focus click",
function(){a(".minidateslector_popup").hide();var b=l.offset().top+l.height()+parseInt(l.css("padding-top"))+parseInt(l.css("padding-bottom"))+6,d=l.offset().left+l.width()/2+parseInt(l.css("padding-left"))+parseInt(l.css("padding-right")),f=h.width();h.css({top:b,left:d-f/2});h.height()+b<a(document).scrollTop()+a(window).height()?h.removeClass("arrow-bottom").addClass("arrow-top"):(b=l.offset().top-h.height()-4,h.css({top:b}).removeClass("arrow-top").addClass("arrow-bottom"));b=l.val();if("time"!=
...
new Date(b);var s=e.getMinutes();if(s){h.find(".hours span,.minutes span,.parts span").removeClass("active");h.find(".minutes span").eq(Math.floor(s/5)).addClass("active");var r=e.getHours();12==c.time_format&&(12<=r?(r-=12,h.find(".part span").eq(1).addClass("active")):h.find(".part span").eq(0).addClass("active"));h.find(".hours span").eq(r).addClass("active")}}catch(u){}}h.show()});h.on("mouseleave",function(){q=setTimeout(function(){h.fadeOut("fast")},1E3)});h.on("mouseenter",function(){console.log("enter");
clearInterval(q);console.log(q)});l.on("keydown",function(){h.hide()});h.find("span").click(function(){a(this).parent().find(".active").removeClass("active");a(this).addClass("active");f(l,h)})})}})(jQuery);